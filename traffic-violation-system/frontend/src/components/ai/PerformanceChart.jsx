...
export default function PerformanceChart({ performance }) {
  // We generate a sliding cache representing recent latency/FPS states
    ...
    if (performance) {
      setHistory((prev) => {
        const next = [...prev, {
          ...
          fps: performance.fps || 30.0,
          latency: performance.inference_time || 12.4
        }];
        if (next.length > 15) next.shift(); // keep last 15 points
        return next;
      });
    }
  }, [performance]);
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 shadow-lg flex flex-col space-y-4">
      <h3 className="font-semibold text-sm text-slate-200 flex items-center space-x-2">
        ...
        <span>Real-Time Performance Logs</span>
      </h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 h-[220px]">
        ...
        <div className="flex flex-col h-full">
          ...
          <ResponsiveContainer width="100%" height="90%">
            <AreaChart data={history} margin={{ top: 5, right: 5, left: -25, bottom: 0 }}>
              ...
              <Area type="monotone" dataKey="latency" name="Latency" stroke="#aa3bff" fill="url(#latencyGlow)" strokeWidth={2} />
              <defs>
        ...
        {/* FPS Area Chart */}
        <div className="flex flex-col h-full">
          <span className="text-[10px] text-slate-500 uppercase font-bold tracking-wider mb-2">Execution Rate (FPS)</span>
          <ResponsiveContainer width="100%" height="90%">
            <AreaChart data={history} margin={{ top: 5, right: 5, left: -25, bottom: 0 }}>
              <CartesianGrid stroke="#1e293b" strokeDasharray="3 3" vertical={false} />
              <XAxis dataKey="time" stroke="#64748b" fontSize={9} tickLine={false} />
              <YAxis stroke="#64748b" fontSize={9} tickLine={false} />
              <Tooltip contentStyle={{ background: '#0f172a', border: '1px solid #1e293b', borderRadius: '8px', fontSize: '10px' }} />
              <Area type="monotone" dataKey="fps" name="Throughput" stroke="#6366f1" fill="url(#fpsGlow)" strokeWidth={2} />
              <defs>
